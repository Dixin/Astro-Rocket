...
Then link to it from any post's MDX with the `<ContentLink>` component — it's available globally, so no import is needed:

```mdx
Read more in the <ContentLink uid="blog/url-structures">URL structures guide</ContentLink>.
```

That renders an ordinary `<a>` to the post's URL. Omit the link text and the target post's own title is used:

```mdx
See also: <ContentLink uid="blog/url-structures" />
```
...
- **Rename-safe.** The `uid` is independent of the filename and slug, so you can rename a post freely and every `<ContentLink>` to it keeps working.
...
The `uid` field is entirely optional — posts without one behave exactly as before; they simply can't be targeted by `<ContentLink>`.
...
If you like Astro Rocket, a [star on GitHub](https://github.com/Dixin/astro-rocket) helps other developers find it. Takes two seconds.

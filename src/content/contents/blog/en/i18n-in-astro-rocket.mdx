...
If you want the broader picture of how the theme is structured first, the <ContentLink uid="blog/configuration-guide">configuration guide</ContentLink> covers `site.config.ts`, themes, and layout switches; the [SEO post](/blog/seo-in-astro-rocket) covers everything in the `<head>` that i18n extends with `hreflang`.
...
2. **Locale-specific slugs** — more natural for SEO (`/nl/blog/hallo-wereld`). Give both posts the same canonical `uid` in frontmatter (the same id `<ContentLink>` uses) and the theme links them correctly across locales and advertises the right URL in `hreflang`.
...
This feature exists, and has matured, because the community asked. It started with [#207](https://github.com/Dixin/Astro-Rocket/issues/207); the auto-loaded dictionaries, locale-validated schemas, non-English default locale, dictionary-driven pages, automatic blog and projects routing, and fully localized chrome all came from real-world reports and requests ([#414](https://github.com/Dixin/Astro-Rocket/issues/414), [#415](https://github.com/Dixin/Astro-Rocket/issues/415), [#418](https://github.com/Dixin/Astro-Rocket/issues/418), [#419](https://github.com/Dixin/Astro-Rocket/issues/419), [#422](https://github.com/Dixin/Astro-Rocket/issues/422), [#437](https://github.com/Dixin/Astro-Rocket/issues/437), [#438](https://github.com/Dixin/Astro-Rocket/issues/438)). Thanks to everyone who filed them. If you build something with the i18n system and hit a rough edge, [open an issue](https://github.com/Dixin/Astro-Rocket/issues) — the next iteration will come the same way.

For now: flip the flag, add your content in a second language, translate the dictionary, and ship it. The complete reference is in the [Internationalization (i18n) section of the README](https://github.com/Dixin/Astro-Rocket#internationalization-i18n) — keep it open while you set up your second locale.
